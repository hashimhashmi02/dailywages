    ...
    <div className="min-h-screen gradient-hero flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="w-16 h-16 gradient-primary rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-500/20">
            <Wrench className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-bold text-white">DailyWages</h1>
          <p className="text-slate-400 text-sm mt-1">Skilled Workers, One Tap Away</p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl">
          ...
          <div className="flex bg-muted rounded-2xl p-1 mb-6">
            {(["customer", "worker", "admin"] as const).map((r) => (
              <button key={r} onClick={() => setMode(r)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold capitalize transition-all ${mode === r ? "bg-white text-foreground shadow-sm" : "text-muted-foreground"}`}>
                {r}
              </button>
            ))}
          </div>
          ...
          <h2 className="text-xl font-bold text-foreground mb-6">
            {mode === "admin" ? "Admin Login" : `Sign in as ${mode === "customer" ? "Customer" : "Worker"}`}
          </h2>
          ...
          <div className="space-y-4">
            ...
            <button className="w-full py-3 border border-border rounded-xl text-sm font-medium hover:bg-muted transition-colors flex items-center justify-center gap-2">
            ...
            {mode !== "admin" && (
              <button className="w-full py-3 border border-border rounded-xl text-sm font-medium hover:bg-muted transition-colors flex items-center justify-center gap-2">
                <Phone className="w-4 h-4" /> Sign in with OTP
              </button>
            )}
          </div>
          ...
          {mode !== "admin" && (
            <p className="text-center text-sm text-muted-foreground mt-6">
              ...
              <Link href={mode === "worker" ? "/worker/onboarding" : "/login"} className="text-primary font-semibold">
                {mode === "worker" ? "Register as Worker" : "Sign up"}
              </Link>
            </p>
          )}
        </div>
